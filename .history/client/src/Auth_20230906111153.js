...
export default function Auth() {
  ...
  const GetDetail = (e) => {
    Axios.get("http://localhost:5000/Userslogin/" + e.target.value)
      .then((res) => {
  ...
  const Login = () => {
    if (!UserDetail) {
      ...
      localStorage.setItem("DepartmentAdmin", UserDetail.Departments);
    ...
    <div className="">
      <div class="text-center mt-10">
        <div class="mb-4">
          ...
          <select className=" w-3/4 mx-10" onClick={GetDetail}>
            {state.map((r) => (
