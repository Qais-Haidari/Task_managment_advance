...
export default function Viewtask(props) {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch((err) => (document.body.innerHTML = err));
    axios
      .get(`http://localhost:5000/Auth/status/${params.TaskID}`)
      .then((res) => {
        for (let index = 0; index < res.data.length; index++) {
          const element = res.data[index].some((value, index, array) => value !== 'NO') return false;
        }
      })
      .catch((err) => (document.body.innerHTML = err));
  }, []);
            ...
            <div className="px-4 sm:px-8">
              <div className="">
                <div className="inline-block min-w-full shadow-md rounded-lg overflow-hidden">
                  <table className="min-w-full leading-normal sm:hidden lg:block">
                    <thead>
                      <tr>
