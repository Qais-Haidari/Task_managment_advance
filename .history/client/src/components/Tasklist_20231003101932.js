...
export default function Tasklist() {
  ...
  useEffect(() => {
    axios
      .get(
        `http://10.0.0.112:5000/escalated/${MomentDate()}/${getCurrentTimeDashboard()}/${localStorage.getItem(
          "First_Name"
    ...
    axios
      .get(
        `http://10.0.0.112:5000/task/${localStorage.getItem(
          "First_Name"
    ...
    axios
      .get(
        `http://10.0.0.112:5000/Department/owner/${localStorage.getItem(
          "Department"
        )}/${getCurrentTimeDashboard()}/${MomentDate()}`
      )
      .then((res) => {
    ...
    axios
      .get(
        `http://10.0.0.112:5000/TaskAuthApprove/${localStorage.getItem(
          "First_Name"
        )}`
      )
      .then((res) => {
    ...
    axios
      .get(
        `http://10.0.0.112:5000/TaskAuthApproveDepartment/${localStorage.getItem(
          "Department"
        )}`
      )
      .then((res) => {
  ...
  const Accpet = (a, b) => {
    axios
      .post(`http://10.0.0.112:5000/AuthUpdateApprove/${b}/${a}`, {})
      .then((res) => {
  ...
  const Reject = (a, b) => {
    axios
      .post(`http://10.0.0.112:5000/AuthUpdateFeedback/${a}/${b}`, {
        feeback: Feedback,
      })
      .then((res) => {
        window.location.reload();
        ...
        <div className="flex sm:flex-col lg:flex-row flex-grow px-10 mt-4 space-x-6 overflow-auto">
          <div className="flex flex-col flex-shrink-0 w-72 sm:ml-6 lg:ml-0">
            <div className="flex items-center flex-shrink-0 h-10 px-2">
              <span className="block lg:text-sm sm:text-lg font-semibold text-white">
                Tasks
              </span>
            </div>
            {state.map((r) => (
              <Link to={`/viewtask/${r.ID}`}>
                <div
                  ...
                  draggable="true"
                >
                  <span className="flex items-center lg:h-auto lg:h-auto lg:px-3 sm:px-4 lg:text-xs sm:text-sm font-semibold text-white-500 bg-white rounded-full">
                    {r.Short_description}
                  </span>
                  ...
                  <div className="flex items-center w-full mt-3 text-xs font-medium text-gray-400">
                    <div className="flex items-center">
                      ...
                      <span className="ml-1 text-white leading-none sm:text-md lg:text-sm">
                        Due {r.end_time}
                      </span>
                    </div>
                  </div>
            ...
            {Department.map((r) => (
              <Link to={`/viewtask/${r.ID}`}>
                <div
                  ...
                  draggable="true"
                >
                  <span className="flex items-center lg:h-6 lg:px-3 sm:px-4 sm:h-12 lg:text-xs sm:text-sm font-semibold text-white-500 bg-white rounded-full">
                    {r.Short_description}
                  </span>
                ...
                {TaskAuth.map((r) => (
                  <details className="group w-full mb-2 border-b-2 border-red-500">
                    ...
                    {r.Attachment === "Yes" ? (
                      <div className="image">
                        <ImageZoom
                          src={`http://10.0.0.112:5000/${r.AttachmentPath}`}
                          alt="Zoom-images"
                          loading="lazy"
                          className="image"
                          zoom="300"
