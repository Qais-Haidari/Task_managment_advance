...
export default function Admin() {
  ...
  const Assign_to_User_ = (e) => {
    if (e.target.value) {
      axios
        .get(`http://10.0.0.146:5000/tasks/${e.target.value}`)
        .then((res) => {
      ...
      axios
        .get(`http://10.0.0.146:5000/Users/${e.target.value}`)
        .then((res) => {
  ...
  const Assign_to_Department_ = (e) => {
    if (e.target.value) {
      axios
        .get(`http://10.0.0.146:5000/tasks2/${e.target.value}`)
        .then((res) => {
          setUserAuth(res.data);
        })
        .catch((err) => (document.body.innerHTML = err));
  ...
  const Escalated_to_User_ = (e) => {
    if (e.target.value) {
      axios
        .get(`http://10.0.0.146:5000/tasks3/${e.target.value}`)
        .then((res) => {
          setUserAuth(res.data);
        })
        .catch((err) => (document.body.innerHTML = err));
  ...
  const Escalated_to_Department_ = (e) => {
    if (e.target.value) {
      axios
        .get(`http://10.0.0.146:5000/tasks4/${e.target.value}`)
        .then((res) => {
          setUserAuth(res.data);
        })
        .catch((err) => (document.body.innerHTML = err));
  ...
  const post = () => {
    if (
      ...
      axios
        .post("http://10.0.0.146:5000/createTask", {
          OnceOFF: onceOFF,
  ...
  //   fetch(
  //     `http://10.0.0.146:5000/image/${ID}/${Task_ID}/${filename}/${getCurrentDate()}/${localStorage.getItem(
  //       "First_Name"
  //     )}`,
  //     {
  //       method: "POST",
