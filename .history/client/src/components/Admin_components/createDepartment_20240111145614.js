...
export default function Admin() {
  ...
  const post = () => {
    if (!name || !email || !start_of_business || !end_of_business) {
      ...
      Axios.post("http://10.0.0.146:5000/createDepartment", {
  ...
  useEffect(() => {
    Axios.get("http://10.0.0.146:5000/department")
      .then((res) => {
        setState(res.data);
      })
        ...
        <div class="container mx-auto px-4 sm:px-8">
          <div class="">
            <div class="inline-block min-w-full shadow-md rounded-lg overflow-hidden">
              <table class="min-w-full leading-normal bg-gray-900">
                <thead>
                  <tr>
                    <th class="px-5 py-3 border-b-2 border-gray-200 bg-gray-900 text-left text-md font-semibold text-white capitalize tracking-wider">
                      Department ID
                    </th>
                    <th class="px-5 py-3 border-b-2 border-gray-200 bg-gray-900 text-left text-md font-semibold text-white capitalize tracking-wider">
                      Department Name
                ...
                <tbody>
                  {state.map((r) => (
                    <tr>
                      <td class="px-5 py-2 border-b border-gray-200 bg-gray-900 text-sm">
                      ...
                      <td class="px-5 py-2 border-b border-gray-200 bg-gray-900 text-white text-sm">
                        <Link to={`/UpdateDepartment/${r.ID}`}>Update</Link>
                      </td>
                      <td class="px-5 py-2 border-b border-gray-200 bg-gray-900 text-sm">
                        <button
                          ...
                          onClick={() => {
                            Axios.post(
                              `http://10.0.0.146:5000/DeleteDepartment/${r.ID}`
                            )
                              .then((res) => {
                                window.location.reload();
                              })
