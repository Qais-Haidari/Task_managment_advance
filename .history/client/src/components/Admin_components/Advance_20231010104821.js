      ...
      .then((res) => {
        ...
        for (let i = 0; i < res.data.length; i++) {
          if (res.data[i].Assign_to_User) {
            AssToUser.push({
              Count: res.data[i].Assign_to_User,
            });
          }
        }
        for (let i = 0; i < res.data.length; i++) {
          if (res.data[i].Assign_to_Department) {
            AssToDep.push({
              Count: res.data[i].Assign_to_Department,
            });
          }
        }
        for (let i = 0; i < res.data.length; i++) {
          if (res.data[i].Escalated_to_User) {
            EscToUser.push({
              Count: res.data[i].Escalated_to_User,
            });
          }
        }
        for (let i = 0; i < res.data.length; i++) {
          if (res.data[i].Escalated_to_Department) {
            EscToDep.push({
              Count: res.data[i].Escalated_to_Department,
            });
          }
        }
        console.log(AssToUser);
        console.log(AssToDep);
        console.log(EscToUser);
        console.log(EscToDep);
      })
