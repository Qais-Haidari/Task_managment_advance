...
export default function Admin() {
  ...
  useEffect(() => {
    axios
      .get(`http://10.0.0.116:5000/task/${id}`)
      .then((res) => {
        setID(res.data.ID);
  ...
  const createTaskAuth = () => {
    if (!Question || !Type) {
      ...
      axios
        .post("http://10.0.0.116:5000/TaskAuth/createTask", {
          Task_ID: ID,
          Type: Type,
          MinValue: Min,
          MaxValue: Max,
  ...
  const Click_ = () => {
    axios
      .get(`http://10.0.0.116:5000/TaskTasks/Auth/${ID}`)
      .then((res) => {
  ...
  const post = () => {
    axios
      .post(`http://10.0.0.116:5000/createTask/update/${id}`, {
