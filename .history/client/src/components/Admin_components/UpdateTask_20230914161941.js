...
export default function Admin() {
  ...
  useEffect(() => {
    axios
      .get(`http://10.0.0.112:5000/task/${id}`)
      .then((res) => {
        setID(res.data.ID);
  ...
  const createTaskAuth = () => {
    if (!Question || !Type) {
      ...
      axios
        .post("http://10.0.0.112:5000/TaskAuth/createTask", {
          Task_ID: ID,
          Type: Type,
          MinValue: Min,
          MaxValue: Max,
  ...
  const Click_ = () => {
    axios
      .get(`http://10.0.0.112:5000/TaskTasks/Auth/${ID}`)
      .then((res) => {
  ...
  const post = () => {
    axios
      .post(`http://10.0.0.112:5000/createTask/update/${id}`, {
          ...
          <div className="mx-8 flex-1">
            <div className="flex flex-wrap -mx-3 mb-6">
              ...
              <div className="w-full md:w-1/2 px-3">
                ...
                <label
                  className="block capitalize tracking-wide text-red-700 font-bold text-sm mb-2"
                  for="grid-last-name"
                >
                  {Priority}
                </label>
