...
export default function Admin() {
  ...
  const [Task_Recurrence_Date, setTask_Recurrence_Date] = useState(0);
  ...
  const [Type__, setType____] = useState("");
  const [Min__, setMin____] = useState("");
  const [Max__, setMax____] = useState("");
  const [Expected__, setExpected____] = useState("");
  const [Question__, setQuestion____] = useState("");
  ...
  const Type___ = (e) => setType____(e.target.value);
  const Min___ = (e) => setMin____(e.target.value);
  const Max___ = (e) => setMax____(e.target.value);
  const Expected___ = (e) => setExpected____(e.target.value);
  const Question___ = (e) => setQuestion____(e.target.value);
  useEffect(() => {
    axios
      .get(`http://10.0.0.116:5000/Departments`)
      .then((res) => {
        setDepartmentFetch(res.data);
      })
      .catch((err) => (document.body.innerHTML = err));
    axios
      .get(`http://10.0.0.116:5000/Users`)
      .then((res) => {
  ...
  const Assign_to_User_ = (e) => {
    if (e.target.value) {
      axios
        .get(`http://10.0.0.116:5000/tasks/${e.target.value}`)
        .then((res) => {
      ...
      axios
        .get(`http://10.0.0.116:5000/Users/${e.target.value}`)
        .then((res) => {
  ...
  const Assign_to_Department_ = (e) => {
    if (e.target.value) {
      axios
        .get(`http://10.0.0.116:5000/tasks2/${e.target.value}`)
        .then((res) => {
          setUserAuth(res.data);
        })
        .catch((err) => (document.body.innerHTML = err));
  ...
  const Escalated_to_User_ = (e) => {
    if (e.target.value) {
      axios
        .get(`http://10.0.0.116:5000/tasks3/${e.target.value}`)
        .then((res) => {
          setUserAuth(res.data);
        })
        .catch((err) => (document.body.innerHTML = err));
  ...
  const Escalated_to_Department_ = (e) => {
    if (e.target.value) {
      axios
        .get(`http://10.0.0.116:5000/tasks4/${e.target.value}`)
        .then((res) => {
          setUserAuth(res.data);
        })
        .catch((err) => (document.body.innerHTML = err));
  ...
  const More_ = () => {
    if (!Question__) {
      ...
      MoreReq.push({
        ...
        MinValue: Min__,
        MaxValue: Max__,
        ExptectedValue: Expected__,
        Questions: Question__,
        User: Assign_to_User,
        Department: Assign_to_Department,
        Date: start_date_time.split(" ")[0],
  ...
  const post = () => {
    if (
      ...
      console.log('run')
      axios
        .post("http://10.0.0.116:5000/createTask", {
          OnceOFF: onceOFF,
          ...
          Task_Recurrence_Date: Task_Recurrence_Date.split('-')[2]
        })
        .then((res) => {
          if (res.data === 'Task Successfully Created') {
  ...
  //   fetch(
  //     `http://10.0.0.116:5000/image/${ID}/${Task_ID}/${filename}/${getCurrentDate()}/${localStorage.getItem(
  //       "First_Name"
  //     )}`,
  //     {
  //       method: "POST",
          ...
          <div className="mx-8 flex-1">
            <div className="flex flex-wrap -mx-3 mb-6">
              ...
              <div className="w-full md:w-1/2 px-3">
                <div className="flex">
                  ...
                  <div>
                    <p className="ml-10 -mt-2 text-blue-600 font-bold">
                      Once Off
                    </p>
                    <Switch
                      checked={enabled}
                ...
                <div className="w-full md:w-1/2 px-3 mt-2">
                  { Task_Recurrence === 'Monthy' ? (<>
                    ...
                    <div class="relative max-w-sm">
                        ...
                        <input type="date" onChange={(e) => {setTask_Recurrence_Date(e.target.value); console.log(e.target.value)}} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Select date" />
                      </div>
                    {/* <select onClick={(e) => {setTask_Recurrence_Date(e.target.value)}} className="mt-2 block w-full rounded-md border-0 py-1.5 pl-3 pr-10 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm sm:leading-6" >
                      <option>1</option>
                      <option>2</option>
                      <option>3</option>
                      <option>4</option>
            ...
            <div className="flex flex-wrap px-3 mt-2 shadow-xl rounded pb-2">
              ...
              {/* {Type === "Attachement" ? (
                <>
                <div className="mr-2">
                    <p className="mb-2 text-white">Image Hint</p>
                    <input
                      className="shadow appearance-none w-32 px-3 text-gray-700 4 focus:outline-none focus:shadow-outline"
                      id="Min"
                      type="file"
              ...
              {Type === "Attachement" || Type === "Yes/NO" ? (
                <></>
              ) : (
                <>
                  <div className="mr-2">
                    <p className="mb-2 text-white">Min Value</p>
                    <input
                  ...
                  <div className="mr-2">
                    <p className="mb-2 text-white">Max Value</p>
                    <input
                  ...
                  <div className="mr-2">
                    <p className="mb-2 text-white">Expected Value</p>
                    <input
              ...
              <div className="mr-2">
                <p className="mb-2 text-white" id="questions">
                  Questions
                </p>
                <input
                  ...
                  <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all w-full">
                    <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4 w-full">
                      <form id="MoreForm">
                        <div className="flex flex-wrap px-3 mt-2 shadow-xl rounded pb-2">
                          <div className="mr-2">
                            <p className="mb-2 text-white">Type</p>
                            <div className="text-xs">
                              <select
                                className="rounded-lg w-40 border-blue-600"
                                id="Type"
                          ...
                          <div className="mt-9">
                            ...
                            <button
                              type="button"
                              className="ml-2 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto"
                              onClick={() => {
                                setMoreReqs([]);
                              }}
                            >
