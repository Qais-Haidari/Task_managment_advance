...
export default function TaskAuth({
  ...
  const action = () => {
    axios
      .post(`http://10.0.0.116:5000/AuthUpdateValue/${ID}/${Task_ID}`, {
        Value: Value,
        Date: getCurrentDate(),
  ...
  const Yes_ = (e) => {
    axios
      .post(`http://10.0.0.116:5000/AuthUpdateYes/${ID}/${Task_ID}`, {
  ...
  if (Type === "Attachement") {
    return (
      <div className="m-5">
        ...
        {MinValue !== "" ? (
          <p className="text-xl text-white mt-2">Min Value {MinValue}</p>
        ) : (
          <></>
        )}
        {MaxValue !== "" ? (
          <p className="text-xl text-white mt-2">Max Value {MaxValue}</p>
        ) : (
          <></>
        )}
        {ExptectedValue !== "" ? (
        ...
        {Feedback !== "" ? (
          <p className="text-xl text-white mt-2">Feeback: {Feedback}</p>
        ) : (
          <></>
        )}
        <p>{status}</p>
    ...
    return (
      <div className="m-5">
        <h1 className="text-lg text-green-500 ">{Questions}</h1>
        {MinValue !== "" ? (
          <p className="text-xl text-white mt-2">Min Value {MinValue}</p>
        ) : (
          <></>
        ...
        <div className="flex-col max-w-xs flex">
          <div class="flex items-center mb-4" className="mt-2">
            ...
            <label
              for="default-radio-1"
              class="mr-2 ml-2 text-sm font-medium text-white dark:text-gray-300"
            >
              Yes
            </label>
            <input
    ...
    return (
      <div className="m-5">
        <h1 className="text-lg text-white ">{Questions}</h1>
        {MinValue !== "" ? (
          <p className="text-xl text-white mt-2">Min Value {MinValue}</p>
        ) : (
          <></>
        ...
        {Feedback !== "" ? (
          <p className="text-xl text-white sm:text-md mt-2">
            Feeback: {Feedback}
          </p>
        ) : (
          <></>
