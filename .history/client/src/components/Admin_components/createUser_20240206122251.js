...
export default function Admin() {
  ...
  const post = () => {
    ...
    if (
      ...
      console.log(DepartmentsAdmin)
    axios
      .post("http://10.0.0.116:5000/createUser", {
        ...
        DepartmentsAdmin: DepartmentsAdmin === true ? Department : 'NO Admin',
        Level: Level,
        IC: IC,
      })
      .then((res) => {
            ...
            <div className="w-full md:w-1/2 px-3">
              ...
              <input type="checkbox" onChange={(e) => { 
                if (e.target.checked === false) {
                  setDepartmentsAdmin(false)
                } else {
                  setDepartmentsAdmin(true)
                }
               }} />
            </div>
            <div className="w-full md:w-1/2 px-3">
              <p className="block capitalize mt-2 tracking-wide text-blue-700 font-bold text-sm mb-2">
                  Authorization Level ( 1: Admin User, 2: Normal User )
              </p>
              <select
                onClick={Level__}
                className="appearance-none block w-full  text-gray-700 border border-gray-200 rounded py-2 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                        ...
                        <td class="px-5 py-2 border-b border-gray-200 text-white bg-gray-900 text-sm">
                          <button
                            ...
                            onClick={() => {
                              axios
                                .post(
                                  `http://10.0.0.116:5000/DeleteUser/${r.ID}`
                                )
                                .then((res) => {
                                  window.location.reload();
                                })
