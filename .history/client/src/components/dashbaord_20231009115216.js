...
export default function Tasklist() {
  ...
  useEffect(() => {
    axios
      .get(`http://10.0.0.112:5000/task/dashboard/get/new/${MomentDate()}`)
      .then((res) => {
        setunAction(res.data);
      })
      .catch((err) => (document.body.innerHTML = err));
    // axios
    //   .get(`http://10.0.0.112:5000/task/dashboard/complete/new/${MomentDate()}`)
    //   .then((res) => {
    //     setComplete(res.data);
    //   })
    //   .catch((err) => (document.body.innerHTML = err));
    axios
      .get(`http://10.0.0.112:5000/task/dashboard/AuthTask/new/${MomentDate()}`)
      .then((res) => {
        setTaskAuth(res.data);
      })
      .catch((err) => (document.body.innerHTML = err));
    axios(
      `http://10.0.0.112:5000/task/dashboard/Department/${MomentDate()}/${getCurrentTimeDashboard()}`
    )
      .then((res) => {
    ...
    axios
      .get(
        `http://10.0.0.112:5000/task/dashboard/Escalated/${MomentDate()}/${getCurrentTimeDashboard()}`
      )
      .then((res) => {
      ...
      <div>
        <div class="flex flex-col w-screen h-screen overflow-auto text-black">
          ...
          <div class="flex flex-grow px-10 mt-4 space-x-6 overflow-auto">
            <div class="flex flex-col flex-shrink-0 w-96">
              <div className="flex flex-col flex-shrink-0 w-96">
                <div className="flex items-center flex-shrink-0 h-10 px-2">
                  <span className="block text-sm font-semibold text-white">
                    InComplete
                  </span>
                  <span className="block text-md ml-60 font-semibold text-red-600">
                    {unAction.length}
                  </span>
                </div>
                {unAction.map((r) => (
            ...
            <div className="flex flex-col flex-shrink-0 w-96">
              <div className="flex items-center flex-shrink-0 h-10 px-2">
                ...
                <span className="block text-md ml-52 font-semibold text-red-600">
                  {Department.length}
                </span>
              </div>
              {Department.map((r) => (
              ...
              <div class="flex flex-col flex-shrink-0 w-96">
                <div className="flex flex-col flex-shrink-0 w-96">
                  <div className="flex items-center flex-shrink-0 h-10 px-2">
                    ...
                    <span className="block text-md ml-32 font-semibold text-red-600">
                      {TaskAuth.length}
                    </span>
                  </div>
                  {TaskAuth.map((r) => (
