...
export default function Tasklist() {
  ...
  const Accpet = () => {
    axios
      .post(
        `http://localhost:5000/AuthUpdateApprove/${
          document.getElementById("HIdden").value
        }/${document.getElementById("HIdden_Task_ID").value}`,
        {}
      )
      .then((res) => {
  ...
  const Reject = () => {
    axios
      .post(
        `http://localhost:5000/AuthUpdateFeedback/${
          document.getElementById("HIdden").value
        }`,
        {
          feeback: Feedback,
            ...
            <div className="flex flex-col flex-shrink-0 w-6/12">
              ...
              <div className="flex flex-col pb-2 overflow-auto">
                <div className="relative flex flex-col items-start p-4 mt-3 bg-white rounded-lg cursor-pointer bg-opacity-90 group hover:bg-opacity-100">
                  {TaskAuth.map((r) => (
                    <details className="group w-full border border-spacing-2 p-2">
                      <summary className="flex justify-between items-center mb-5  font-medium cursor-pointer list-none">
