...
export default function Profile() {
  ...
  const Update = () => {
    ...
    axios
      .post(`http://localhost:5000/User/update/${localStorage.getItem("ID")}/${status}`)
      .then((res) => {
        Login()
      })
      .catch((err) => {});
      ...
      <div className="">
        <div className=" mx-10 mt-16 shadow-xl rounded-lg text-white bg-gray-900">
          ...
          <div className="text-center text-xl mt-2">
            <h2 className="font-semibold text-red">{localStorage.getItem("First_Name")}</h2>
          </div>
          <div className=" p-3 shadow-sm rounded-sm">
            <div className="text-white">
              <div className="grid md:grid-cols-2 text-md">
                <div className="grid grid-cols-2">
                  <div className="px-4 py-2 font-semibold text-red">User ID</div>
                  <div className="px-4 py-2">{localStorage.getItem("ID")}</div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="px-4 py-2 font-semibold text-red">User Status</div>
                  <div className="px-4 py-2">{localStorage.getItem("Status")}</div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="px-4 py-2 font-semibold text-red text-red">First Name</div>
                  <div className="px-4 py-2">
                    {localStorage.getItem("First_Name")}
                  </div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="px-4 py-2 font-semibold text-red text-red">2IC</div>
                  <div className="px-4 py-2">
                    {localStorage.getItem("IC")}
                  </div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="px-4 py-2 font-semibold text-red text-red">Last Name</div>
                  <div className="px-4 py-2">
                    {localStorage.getItem("Last_Name")}
                  </div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="px-4 py-2 font-semibold text-red text-red">Email</div>
                  <div className="px-4 py-2 sm:break-words">
                    {localStorage.getItem("Email")}
                  </div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="px-4 py-2 font-semibold text-red text-red">Phone</div>
                  <div className="px-4 py-2">{localStorage.getItem("Phone")}</div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="px-4 py-2 font-semibold text-red text-red">Start of Business</div>
                  <div className="px-4 py-2">
                    {localStorage.getItem("Start_of_business")}
                  </div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="px-4 py-2 font-semibold text-red-600 text-red">End Of Business</div>
                  <div className="px-4 py-2">
                    {localStorage.getItem("End_of_business")}
                  </div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="px-4 py-2 font-semibold text-red">Is Admin</div>
                  <div className="px-4 py-2">
                    <div className="text-white">
