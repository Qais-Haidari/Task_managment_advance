...
export default function Tasklist({ Department }) {
  ...
  useEffect(() => {
    axios
      .get(
        `http://10.0.0.116:5000/task/advancedashboard/Department/${MomentDate()}/${Department}`
      )
      .then((res) => {
        setstate(res.data);
        let state = res.data;
        ...
        axios
          .post(
            `http://10.0.0.116:5000/task/advancedasbhboard/TaskID/${MomentDate()}/${Department}`,
        ...
        <tbody class="text-sm divide-y divide-gray-100 h-full">
          <tr>
            <td class="p-2 whitespace-nowrap">
              <div class="">
                <div class="text-lg text-left ">{Department}</div>
              </div>
            </td>
            <td class="p-2 whitespace-nowrap">
                            ...
                            {!r.AttachmentPath || !r.value ? (
                              ...
                              <>
                                {r.Attachment === "Yes" && r.AttachmentPath ? (
                                  <div className="image">
                                    <ImageZoom
                                      src={`http://10.0.0.116a:5000/${r.AttachmentPath}`}
                                      alt="Zoom-images"
                                      loading="lazy"
                                      className="image"
                                      zoom="300"
