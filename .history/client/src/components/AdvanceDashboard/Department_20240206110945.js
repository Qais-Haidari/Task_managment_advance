...
export default function Tasklist({ Department }) {
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost:5000/task/advancedashboard/Department/${MomentDate()}/${Department}`
      )
      .then((res) => {
        setstate(res.data);
        let state = res.data;
        ...
        axios
          .post(
            `http://localhost:5000/task/advancedasbhboard/TaskID/${MomentDate()}/${Department}`,
                            ...
                            {!r.AttachmentPath || !r.value ? (
                              ...
                              <>
                                {r.Attachment === "Yes" && r.AttachmentPath ? (
                                  <div className="image">
                                    <ImageZoom
                                      src={`http://localhosta:5000/${r.AttachmentPath}`}
                                      alt="Zoom-images"
                                      loading="lazy"
                                      className="image"
                                      zoom="300"
