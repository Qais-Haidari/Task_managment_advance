...
export default function Tasklist() {
  ...
  const filterApply = () => {
    ...
    // axios
    //   .get(`http://localhost:5000/task/dashboard/Monitor/${document.getElementById('two').value}/${document.getElementById('one')}/notEST/${MomentDate()}/${getCurrentTimeDashboard()}`)
    //   .then((res) => {
    //     setunAction(res.data);
    //   })
    //   .catch((err) => (document.body.innerHTML = err));
    ...
    //   axios(
    //     `http://localhost:5000/task/dashboard/Monitor/${document.getElementById('two').value}/${document.getElementById('one')}/${MomentDate()}/${getCurrentTimeDashboard()}`
    //   )
    //     .then((res) => {
    //       setEscalated(res.data);
    //     })
  ...
  return (
    <>
      ...
      <div className=" w-screen h-screen overflow-auto text-black">
        ...
        <div className="flex">
        <div className="flex-1 px-2 mt-4 overflow-auto">
          <p className="text-white text-md" >Tasks ( <span className="text-red" >{unAction.length}</span> )</p>
          <div className="">
            {unAction.map((r) => (
              <div
        ...
        <div className="flex-1 px-2 mt-4 overflow-auto">
        <p className="text-white text-md">Tasks Escalated ( {Escalated.length} )</p>
          <div className="">
            {Escalated.map((r) => (
              <div
                className={`relative flex flex-col items-start p-4 mt-3 rounded-lg cursor-pointer bg-opacity-90 group hover:bg-opacity-100 
