...
export default function Admin() {
  ...
  useEffect(() => {
    axios
      .get(`http://10.0.0.146:5000/PrimaryTasks/${id}`)
      .then((res) => {
        setID(res.data.ID);
  ...
  const createTaskAuth = () => {
    if (!Question || !Type) {
      ...
      axios
        .post("http://10.0.0.146:5000/Priamry/createTask", {
          Task_ID: ID,
          Type: Type,
          MinValue: Min,
          MaxValue: Max,
  ...
  const delete_ = () => {
    axios
      .post(`http://10.0.0.146:5000/DeletePrimaryTask/${id}`, {
  ...
  const post = () => {
    axios
      .post(`http://10.0.0.146:5000/PrimaryTask/update/${id}`, {
        Short_description: Short_description,
        Summary: Summary,
        Priority: Priority,
  ...
  const Click_ = () => {
    axios
      .get(`http://10.0.0.146:5000/PrimaryTasks/Auth/${ID}`)
      .then((res) => {
  ...
  const Delete_Auth = (e) => {
    axios
      .get(`http://10.0.0.146:5000/DeletePrimaryTaskAuth/${e}`)
      .then((res) => {
        alert("Deleted");
        window.location.reload();
      })
