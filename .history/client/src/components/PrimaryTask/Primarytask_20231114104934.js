...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:5000/PrimaryTasks/user/${localStorage.getItem(
          "First_Name"
        )}`
      )
      .then((res) => {
  ...
  const Priority_ = (e) => {
    axios
      .get(`http://localhost:5000/PrimaryTasks/priority/${e.target.value}`)
      .then((res) => {
        setstate(res.data);
      })
      .catch((err) => (document.body.innerHTML = err));
  ...
  const TaskRecurrence_ = (e) => {
    axios
      .get(
        `http://localhost:5000/PrimaryTasks/TaskRecurrence/${e.target.value}`
      )
      .then((res) => {
        setstate(res.data);
      })
      ...
      <div className="">
        <div className="">
          <div className="flex ml-2 mt-2">
            ...
            <div>
              <p className="ml-2">TaskRecurrence</p>
              <select
