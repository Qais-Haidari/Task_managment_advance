...
export default function Tasklist() {
  ...
  useEffect(() => {
    ...
    // axios
    //   .get(`http://10.0.0.146:5000/task/dashboard/Monitor/notEST/${MomentDate()}/${getCurrentTimeDashboard()}`)
    //   .then((res) => {
    ...
    //   .catch((err) => (document.body.innerHTML = err));
      ...
      axios(
        `http://10.0.0.146:5000/task/dashboard/Monitor/${MomentDate()}/${getCurrentTimeDashboard()}`
      )
        .then((res) => {
  ...
  const filterApply = () => {
    axios
      .get(`http://10.0.0.146:5000/task/dashboard/Monitor/${document.getElementById('two').value}/${document.getElementById('one').value}/notEST/${MomentDate()}/${getCurrentTimeDashboard()}`)
      .then((res) => {
        setunAction(res.data);
      })
      .catch((err) => (document.body.innerHTML = err));
      axios(
        `http://10.0.0.146:5000/task/dashboard/Monitor/${document.getElementById('two').value}/${document.getElementById('one').value}/${MomentDate()}/${getCurrentTimeDashboard()}`
      )
        .then((res) => {
          setEscalated(res.data);
        })
