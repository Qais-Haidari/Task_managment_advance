...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("Status") === 'false'){
      document.getElementById('usertoggle').setAttribute('checked', )
      console.log('1')
    }
  }, []);
  const Update = () => {
    ...
    // axios
    //   .post(`http://localhost:5000/User/update/${localStorage.getItem("ID")}/${document.getElementById('usertoggle').getAttribute('checked')}/`)
    //   .then((res) => {
    //     // alert("Update");
      ...
      <div className="">
        <div className=" mx-10 mt-16 shadow-xl rounded-lg text-white bg-gray-900">
          ...
          <label className="relative ml-2 mt-4 inline-flex items-center cursor-pointer">
            <input type="checkbox" id="usertoggle" onClick={(e) => console.log(e.target.checked)} className="sr-only peer" />
