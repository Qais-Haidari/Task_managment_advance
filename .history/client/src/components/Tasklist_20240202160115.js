...
export default function Tasklist() {
  ...
  useEffect(() => {
      axios.get(`http://10.0.0.116:5000/User/2ic/${localStorage.getItem('IC')}`).then((res) => {
        if (res.data.Status === false) {
          axios
          .get(
            `http://10.0.0.116:5000/escalated/${MomentDate()}/${getCurrentTimeDashboard()}/${res.data.First_Name}/${localStorage.getItem("Department")}`
          )
          .then((res) => {
      ...
      axios
        .get(
          `http://10.0.0.116:5000/task/${res.data.First_Name}/${getCurrentTimeDashboard()}/${MomentDate()}`
        )
        .then((res) => {
          setICstate(res.data);
        })
        ...
        axios
          .get(
            `http://10.0.0.116:5000/TaskAuthApprove/${res.data.First_Name}`
          )
          .then((res) => {
    ...
    axios
      .get(
        `http://10.0.0.116:5000/escalated/${MomentDate()}/${getCurrentTimeDashboard()}/${localStorage.getItem(
          "First_Name"
    ...
    axios
      .get(
        `http://10.0.0.116:5000/task/${localStorage.getItem(
          "First_Name"
      ...
      axios
        .get(
          `http://10.0.0.116:5000/TaskAuthApprove/${localStorage.getItem(
            "First_Name"
          )}`
        )
        .then((res) => {
    ...
    axios
      .get(
        `http://10.0.0.116:5000/Department/owner/${localStorage.getItem(
          "Department"
        )}/${getCurrentTimeDashboard()}/${MomentDate()}`
      )
      .then((res) => {
    ...
    axios
      .get(
        `http://10.0.0.116:5000/TaskAuthApproveDepartment/${localStorage.getItem(
          "Department"
        )}`
      )
      .then((res) => {
  ...
  const Accpet = (a, b) => {
    axios
      .post(`http://10.0.0.116:5000/AuthUpdateApprove/${b}/${a}`, {})
      .then((res) => {
  ...
  const Reject = (a, b) => {
    axios
      .post(`http://10.0.0.116:5000/AuthUpdateFeedback/${a}/${b}`, {
        feeback: Feedback,
      })
      .then((res) => {
        window.location.reload();
                ...
                {TaskAuth.map((r) => (
                  <details className="group w-full mb-2 border-b-2 border-red-500">
                    ...
                    {r.Attachment === "Yes" ? (
                      <div className="image">
                        <ImageZoom
                          src={`http://10.0.0.116:5000/${r.AttachmentPath}`}
                          alt="Zoom-images"
                          loading="lazy"
                          className="image"
                          zoom="300"
