...
export default function Auth() {
  ...
  const GetDetail = (e) => {
    Axios.get("http://10.0.0.112:5000/Userslogin/" + e.target.innerText)
      .then((res) => {
        localStorage.setItem("Status", req.data.Status);
        localStorage.setItem("ID", req.data.ID);
        localStorage.setItem("First_Name", req.data.First_Name);
        localStorage.setItem("Last_Name", req.data.Last_Name);
        localStorage.setItem("Email", req.data.Email);
        localStorage.setItem("Phone", req.data.Phone);
        localStorage.setItem("Start_of_business", req.data.Start_of_business);
        localStorage.setItem("End_of_business", req.data.End_of_business);
        localStorage.setItem("Is_Admin", req.data.Is_Admin);
        localStorage.setItem("Department", req.data.Departments);
        localStorage.setItem("DepartmentAdmin", req.data.DepartmentsAdmin);
