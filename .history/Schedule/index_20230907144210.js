...
const Create = async () => {
  ...
  for (let index = 0; index < Daily_PrimaryTask.length; index++) {
    ...
    for (let index1 = 0; index1 < Daily_PrimaryTask.length; index1++) {
      Daily_PriamryTask_Auth[index1] = await PrimaryTaskAuth.findOne({
        Task_ID: Daily_PrimaryTask[index1].ID,
      });
      console.log(Daily_PriamryTask_Auth[index1].Questions);
      //   await TaskAuth.create({
      //     ID: uuidv4(),
      //     Task_ID: ID,
      //     Type: Daily_PriamryTask_Auth.Type,
      //     MinValue: Daily_PriamryTask_Auth.MinValue,
      //     MaxValue: Daily_PriamryTask_Auth.MaxValue,
      //     ExptectedValue: Daily_PriamryTask_Auth.ExptectedValue,
      //     Questions: Daily_PriamryTask_Auth.Questions,
      ...
      //     ApproveBy: Daily_PriamryTask_Auth.ApproveBy,
      //   });
    }
  }
